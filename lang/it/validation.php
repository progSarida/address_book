...
return [
    ...
    */
    ...
    | "campo.regola", che ha la precedenza su tutti i messaggi generici qui
    | sopra.
    |
    | I form Filament validano usando lo state path del form come prefisso,
    | quindi i campi delle pagine risorsa si trovano sotto "data.". I campi
    | del contatto (ContactResource) sono resi con ->hiddenLabel(): non avendo
    | label, il segnaposto :attribute verrebbe sostituito con il nome della
    | colonna del database ("surname", "phone_1", ...). Per questi campi il
    | messaggio riporta quindi direttamente il testo del ->placeholder(), che
    | e' il nome che l'utente vede a video.
    |
    | I campi dei referenti hanno una label vera e restano sotto
    | "data.referents.<uuid>.*", quindi non vengono intercettati da queste
    | chiavi e continuano a usare i messaggi generici.
    |
    */
    ...
    'custom' => [
        'data' => [
            ...
            // Contatto
            'title' => [
                'in' => 'Il valore selezionato per Titolo non è valido.',
            ],
            'surname' => [
                'required' => 'Il campo Cognome/Denominazione è obbligatorio.',
                'max' => 'Il campo Cognome/Denominazione non deve contenere più di :max caratteri.',
            ],
            'name' => [
                'max' => 'Il campo Nome non deve contenere più di :max caratteri.',
            ],

            // Indirizzo
            'toponym' => [
                'in' => 'Il valore selezionato per Toponimo non è valido.',
            ],
            'addr' => [
                'max' => 'Il campo Indirizzo non deve contenere più di :max caratteri.',
            ],
            'num' => [
                'max' => 'Il campo Civico non deve contenere più di :max caratteri.',
            ],
            'apart' => [
                'max' => 'Il campo Interno non deve contenere più di :max caratteri.',
            ],
            'cap' => [
                'max' => 'Il campo Cap non deve contenere più di :max caratteri.',
            ],
            'province' => [
                'max' => 'Il campo Provincia non deve contenere più di :max caratteri.',
            ],

            // Contatti
            'phone_1' => [
                'max' => 'Il campo Telefono 1 non deve contenere più di :max caratteri.',
            ],
            'phone_2' => [
                'max' => 'Il campo Telefono 2 non deve contenere più di :max caratteri.',
            ],
            'smart_1' => [
                'max' => 'Il campo Cellulare 1 non deve contenere più di :max caratteri.',
            ],
            'smart_2' => [
                'max' => 'Il campo Cellulare 2 non deve contenere più di :max caratteri.',
            ],
            'email_1' => [
                'max' => 'Il campo Email 1 non deve contenere più di :max caratteri.',
            ],
            'email_2' => [
                'max' => 'Il campo Email 2 non deve contenere più di :max caratteri.',
            ],
            'site' => [
                'max' => 'Il campo Sito non deve contenere più di :max caratteri.',
            ],
        ],
    ],
